...
describe('AssignUsersModalComponent', () => {
  ...
  let mockedModal = {
    ...
    fundsAssign: [{ assignedTo: [{ userId: 123 }, { userId: 2202 }] }],
    idFund: 1,
  };
  ...
  it('ngOnInit method should call getListUsers', () => {
    spyOn(component, 'getListUsers');
    component.ngOnInit();
    expect(component.getListUsers).toHaveBeenCalled();
  });
  it('should get and set the list of users oninit', () => {
    fixture.detectChanges();
    let listUsers: any = [];
    ...
    mockedListofUsers.data.forEach((item) => {
      const eachitem: any = {
        ...
        approved: true,
        check:
          component.modalDetails.fundsAssign[0].assignedTo.findIndex(
  ...
  it('should call createListUserToAddRowData', () => {
    spyOn(component, 'createListUserToAddRowData');
    component.completedData = [];
  ...
  it('onClickYes method should call addUsersToFund and return Yes after click ok', () => {
    ...
    expect(component['assignmentsService'].addUsersToFund).toHaveBeenCalledWith(
      mockedModal.idFund,
      { userIds: [123, 2202] }
    );
