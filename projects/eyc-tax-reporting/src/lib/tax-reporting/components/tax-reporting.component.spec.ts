...
const productionCycleServiceStub = jasmine.createSpyObj(
  'productcyclesService',
  [
    'getProductionCycles',
...
describe('TaxReportingComponent', () => {
  ...
  beforeEach(() => {
    ...
    productionCycleServiceStub.getProductionCycles.and.returnValue(
      of(mockedReports)
    );
  ...
  it('should check the service', () => {
    expect(
      productionCycleService instanceof ProductionCycleService
    ).toBeTruthy();
    expect(
      managementReportsService instanceof ManagementReportsService
    ).toBeTruthy();
  });
  ...
  it('should get completed filing data', () => {
    ...
    resp['data'].forEach((item) => {
      ...
      reportResp[0].data.length === 0
        ? (component.noActivatedDataAvilable = true)
  ...
  it('getCompletedProductCyclesData should get the product cycles', () => {
    let resp = mockedReports;
    component.calledProductCyclesList = false;
    let mockedProductCycles: any = [];
    spyOn(
      component['productcyclesService'],
      'getProductionCycles'
    ).and.returnValue(of(mockedReports));
    spyOn(component, 'createHistoryRowData');
    component.getCompletedProductCyclesData();
        ...
        dataToChart: [
          {
            'in EY tax preparation':
              item.fundCount != null
                ? item.fundCount.fundCountByStatus[0].fundCount
                : 0,
            'in client review':
              item.fundCount != null
                ? item.fundCount.fundCountByStatus[1].fundCount
                : 0,
            'Approved by client':
              item.fundCount != null
                ? item.fundCount.fundCountByStatus[2].fundCount
                : 0,
          },
        ],
      };
      mockedProductCycles.push(eachitem);
    });
    expect(component.completedReports).toEqual(mockedProductCycles);
    expect(component.createHistoryRowData).toHaveBeenCalled();
  });
  it('createHistoryRowData method should set row data', () => {
    let mockedRowData: any = [];
    let mockedFilteredReports: any = [];
    mockedReports.data.forEach((item) => {
      const eachitem: any = {
        name: item.name,
        id: item.id,
      ...
      mockedFilteredReports.push(eachitem);
    });
    component.completedReports = mockedFilteredReports;
    mockedFilteredReports.forEach((filing) => {
      mockedRowData.push({
        name: filing.name,
        id: filing.id,
        ...
        dataToChart: [
          {
            'in EY tax preparation':
              filing.dataToChart[0]['in EY tax preparation'],
    ...
    component.createHistoryRowData();
    ...
    result$.subscribe((resp) => {
      resp['data'].forEach((item) => {
        const eachitem: any = {
          ...
          id: item.id,
          totalFunds:
            item.fundCount != null ? item.fundCount.fundTotalCount : 0,
          dataToChart: [
            {
              'in EY tax preparation':
                item.fundCount != null
  ...
  it('getProdCycleDetail method should navigate to cycle-details path with row id and row name',()=>{
    spyOn(component['router'],'navigate');
    const url =  ['cycle-details', 1,'test'];
    let row = {id: 1, name: 'test'}
    component.getProdCycleDetail(row);
