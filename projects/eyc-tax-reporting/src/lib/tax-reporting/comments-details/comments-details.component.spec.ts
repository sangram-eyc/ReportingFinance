...
describe('CommentsDetailsComponent', () => {
  ...
  let mockedCommentDetailsExpand = {
    success: true,
    message: '',
    corelationId: null,
    data: {
  ...
  let commentServiceStub = {
    ...
    getCommentExpandDetails: () => {
      return of(mockedCommentDetailsExpand);
    },
  };
  ...
  it('ngOnInit method set route params, call getCommentsList function and set comment details', () => {
    ...
    let openByEY = mockedCommentDetailPerProdCycle.data.open.find(
      (status) => status.target === 'EY'
    );
    let openByClient = mockedCommentDetailPerProdCycle.data.open.find(
      (status) => status.target === 'CLIENT'
    );
    let AcceptedByEY = mockedCommentDetailPerProdCycle.data.closed.find(
      (status) => status.target === 'EY' && status.status === 'ACCEPTED'
    );
    let DeclinedByEY = mockedCommentDetailPerProdCycle.data.closed.find(
      (status) => status.target === 'EY' && status.status === 'DECLINED'
    );
    let AcceptedByClient = mockedCommentDetailPerProdCycle.data.closed.find(
      (status) => status.target === 'CLIENT' && status.status === 'ACCEPTED'
    );
    let DeclinedByClient = mockedCommentDetailPerProdCycle.data.closed.find(
      (status) => status.target === 'CLIENT' && status.status === 'DECLINED'
    );
    let mockedTextCountNumber =
      Number(mockedCommentDetailPerProdCycle.data.totalOpen) +
      Number(mockedCommentDetailPerProdCycle.data.totalClosed);
    let mockedTotalOpenedCommentsDetails: any = [
      {
    ...
    let mockedTotalClosedCommentsDetails: any = [
      {
        label: 'Accepted',
        value:
    ...
    expect(component.totalOpenedComments).toEqual(
      mockedCommentDetailPerProdCycle.data.totalOpen
    );
    expect(component.totalClosedComments).toEqual(
      mockedCommentDetailPerProdCycle.data.totalClosed
    );
    expect(component.textCountNumber).toEqual(mockedTextCountNumber);
    expect(component.totalOpenedCommentsDetails).toEqual(
      mockedTotalOpenedCommentsDetails
    );
    expect(component.totalClosedCommentsDetails).toEqual(
      mockedTotalClosedCommentsDetails
    );
  });
  ...
  it('should call getCommentsList from the service and call createCommentsRowData', () => {
    ...
    let mockedCompletedComments: any = [];
    ...
    expect(component.createCommentsRowData).toHaveBeenCalledWith(
      mockedCompletedComments
    );
  });
  ...
  it('createCommentsRowData method should populate rowData and call isToggleLeftDisabled function', () => {
    ...
    mockedCompletedComments.forEach((item) => {
      mockedRowData.push({
        ...
        tagsToSearch:
          item.priority == 1
            ? component.splitTags(item.tags, true)
            : component.splitTags(item.tags, false),
      });
    });
  ...
  it('showMyAssignedFunds method should call gridFilter and storage userEmail', () => {
    ...
    expect(component.gridFilter).toHaveBeenCalledWith(fakeStore.userEmail);
  ...
  it('splitTags method should split users', () => {
  ...
  it('openDialog method should call commentService and expandDetails', () => {
    spyOn(
      ...
      'getCommentExpandDetails'
    ).and.returnValue(of(mockedCommentDetailsExpand));
    spyOn(component, 'getCommentsList');
    let mockData = {
      afterClosed: () => of({}),
    };
    ...
    let item = mockedCommentDetailsExpand['data'].taskResponse;
    let _replies = mockedCommentDetailsExpand['data'].repliesDetail;
    const comment: any = {
      id: item.id,
      entityId: item.entityId,
      description: item.description,
    ...
    expect(
      component['commentService'].getCommentExpandDetails
    ).toHaveBeenCalledWith(1);
    expect(component.getCommentsList).toHaveBeenCalled();
    expect(component['dialog'].open).toHaveBeenCalled();
  });
  it('ngOnDestroy method should unsubscribe getCommentsDetails', () => {
    spyOn(component.getCommentsDetails, 'unsubscribe');
    component.ngOnDestroy();
    expect(component.getCommentsDetails.unsubscribe).toHaveBeenCalledTimes(1);
  });
});
