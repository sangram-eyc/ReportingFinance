...
describe('CommentsPageComponent', () => {
  ...
  beforeEach(async(() => {
    ...
    const mockedParams = of({name: 'testName', id : '1', type: '1', prodCycleName: 'test', status:'approved', openCommentsEY: 1, openCommentsClient: 1, cycleId: '1'});
    const activatedRoute = {params :mockedParams};
  ...
  it('should check the service', () => {
    expect(commentService instanceof TaxCommentService).toBeFalsy();
  });
  ...
  it('getComments method get taskData by fundId, and call updateHasOpenComments and countStatusTasks functions', () => {
    ...
    mockedCommentList['data'].forEach((item: any) => {
      const eachitem: any = {
        ...
        attachmentsCount:
          item.attachmentsCount == undefined || item.attachmentsCount == null
            ? 0
    ...
    let filteredComments = mockedCompletedComments.filter(
      (item) =>
        item.target.toUpperCase() === mockedType.toUpperCase() &&
  ...
  it('showOpenComments method should show or hide comments depending on the list of completedComments', () => {
    ...
    expect(component.searchActiveComments).toHaveBeenCalledWith(
      mockedTextToFind
    );
  });
  ...
  it('updateHasOpenComments method should set hasOpenComments variable and call isOpenStatus function', () => {
    ...
    expect(component.updateHasOpenComments).toHaveBeenCalledWith(
      mockedCommenctStatus
    );
  });
  ...
  it('addCommentToFund method should call getComments and set variables afterClosed', () => {
    let mockResult = {
      button: 'Post',
      data: {
        assignTo: 'abcd',
  ...
  it('approveToFund method should call putApproveEntities and set variables afterClosed', () => {
    let mockResult = {
      button: 'Continue',
      data: {
        assignTo: 'abcd',
    ...
    component.approveToFund();
    expect(component.toastSuccessMessage).toEqual("Approved successfully");
    expect(component.isApproved).toEqual(true);
    expect(component.showToastAfterSubmit).toEqual(true);
    expect(component['productcyclesService'].putApproveEntities).toHaveBeenCalledWith(mockBody);
    expect(component['dialog'].open).toHaveBeenCalled();
  });
  ...
  it(`searchActiveComments method should filter the completedComments for a match`, () => {
    ...
    let mockedFilteredComments = mockedCompletedComments.filter((element) => {
      return (
        filtersArray.filter((filterElement) => {
  ...
  it('countStatusTasks method should search data on paste', () => {
    ...
    let mockedFilteredComments = mockedCompletedComments.filter(
      (item) =>
        item.target.toUpperCase() === mockedType.toUpperCase() &&
        item.status.toUpperCase() === 'OPEN'
    );
        ...
        if (item.fundDTO.totalComments > 0) {
          item.tasks.forEach((itemTask) => {
            const eachitem: any = {
              ...
              replyCount: item.replyCount,
              attachmentsCount:
                item.attachmentsCount == undefined ||
