  ...
  loadAuthDetails() {
    return new Promise((resolve, reject) => {
      ...
      this.http.get(`${authorization.auth_Details}`).subscribe(res => {
        ...
        authConfig.redirectUri = environment.production ? environment.SERVICE_URL +  res['redirectUrl'] : this.API_ENDPOINT + res['redirectUrl'];
        authConfig.clientId = res['clientId'];
        authConfig.silentRefreshRedirectUri = environment.production ? environment.SERVICE_URL +  res['silentRefreshRedirectUri'] : this.API_ENDPOINT + res['silentRefreshRedirectUri'];
