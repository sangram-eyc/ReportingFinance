    ...
    OnDestroy
{
  ...
  @ViewChild('notificationicon', { static: false })
  ...
  checkTimeOut() {
    ...
    this.timeoutWarnDownloads = setTimeout(() => {
      if (sessionStorage.getItem('pendingDownloadsBulk') != null) {
        this.pendingDownloads = JSON.parse(
          sessionStorage.getItem('pendingDownloadsBulk')
        );
        if (
          this.settingsService.isUserLoggedin() &&
          this.pendingDownloads.length > 0
        ) {
          this.openPendingDownloadsTaxModal(
            'Warning',
            'A download is in progress and if the session expires while the download is in progress, the download request will not complete.'
          );
        }
      }
  ...
  openErrorModal() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      sessionStorage.setItem(
        'sessionTimeOut',
        sessionStorage.getItem('inActivityTime')
      );
  ...
  ngOnInit(): void {
    ...
    this.moduleLevelPermission.moduleLevelPermisssionDetails.subscribe(
      (res) => {
        setTimeout(() => {
          const uname = res;
          ...
          this.permission.isDataIntake =
            this.moduleLevelPermission.checkPermission('Data Intake');
          this.permission.isAdmin =
            this.moduleLevelPermission.checkPermission('Admin');
          this.permission.isRegReporting =
            this.moduleLevelPermission.checkPermission('Regulatory Reporting');
          this.permission.isTaxReporting =
            this.moduleLevelPermission.checkPermission('Tax Reporting');
          this.permission.isDMS = this.moduleLevelPermission.checkPermission(
            'Data Managed Services'
          );
          this.permission.isEFR = this.moduleLevelPermission.checkPermission(
            'European Fund Reporting'
          );
        }, 100);
      }
    );
  ...
  public notification() {
    ...
    this.isNotificationRead =
      sessionStorage.getItem('isNotificationRead') === 'true';
  ...
  warningMessage(): void {
    const dialogConfirm = this.dialog.open(BulkDownloadModalComponent, {
      ...
      data: {
        header: 'Warning',
        description:
          'The selected files will be compressed into a zip file. You will receive an in-app notification alerting you when the files are ready for download.',
        important:
          'Please note that logging out of the application before files are finished processing will cancel this request.',
  ...
  public outsideClick(event) {
    if (
      ...
      !this.notificationCard.nativeElement.contains(event.target) &&
  ...
  onMessage(event) {
    if (
      ...
      if (event.data.includes('#access_token=')) {
        setTimeout(() => {
          this.settingsService.setIdToken(
  ...
  bulkDownloadWarnings(objectFromServer: string) {
    try {
      ...
      if (fails > 0) {
        ...
        this.openPendingDownloadsTaxModal(
          'Error',
          "Some of selected files had errors, so that can't be downloaded. Please reload the page and try again the missing file(s) from : " +
            fundsNames +
            '.'
        );
      }
      if (sessionStorage.getItem('pendingDownloadsBulk') != null) {
        ...
        const startDownloading = this.pendingDownloads.find(
          (element) => element == id
        );
        this.pendingDownloadsNew = this.pendingDownloads.filter(
          (item) => item != id
        );
        sessionStorage.setItem(
          'pendingDownloadsBulk',
          JSON.stringify(this.pendingDownloadsNew)
        );
